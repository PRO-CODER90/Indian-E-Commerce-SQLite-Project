...
> Replace the lab-specific values (e.g. `<REGION>`, `<CLOUD_SQL_INSTANCE_ID>`, `<QWIKLABS_USER>`, `<TABLE_NAME>`) with the values shown in your own lab panel before following along.
...
## ⚠️ About the `replication_admin` Username
...
> **`replication_admin` is the exact username required by this lab's grader - do NOT change it.** The Database Migration Service connection profile and all SQL permission grants must use this exact username. It does not vary between lab sessions.
...
Copy the **📋 Quick Copy SQL blocks** found after Step 12 below, paste them into the tool, replace `replication_admin` with your desired username, then copy the result and run it. This avoids manually editing 45+ occurrences by hand.
...
```bash
sudo apt install -y postgresql-<VERSION>-pglogical
```
...
> **⚠️ The username `replication_admin` is fixed for this lab - do NOT change it.**
...
- `replication_admin` has all permissions (Steps 11 and 12)
...
Go to: **Cloud SQL → `<CLOUD_SQL_INSTANCE_ID>` → Connections → Networking**
...
Go to: **Cloud SQL → `<CLOUD_SQL_INSTANCE_ID>` → Users → Add user account**
...
Go to: **Cloud SQL → `<CLOUD_SQL_INSTANCE_ID>` → Overview**
...
Replace `<TABLE_NAME>` with the table specified by the lab, and `<QWIKLABS_USER>` with the exact Qwiklabs user account name from your lab panel.
...
Go to: **Cloud SQL → `<CLOUD_SQL_INSTANCE_ID>` → Overview → Edit → Data Protection**
...
Replace `<YOUR_MIGRATED_INSTANCE_ID>` with the actual instance ID from your lab panel.
...
Replace `<YOUR_SAVED_UTC_TIMESTAMP>` with the timestamp from Step 28. Example:
...
| Item                  | Value                  |
| --------------------- | ---------------------- |
| Migration username    | `replication_admin`      |
